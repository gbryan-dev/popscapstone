...
if ($result->num_rows > 0) {
    ...
    while ($doc = $documents->fetch_assoc()) {
        ...
        $uniqueId = 'doc_' . $doc['id'];
         ...
         // onclick="window.open(\'elametadata?file=' . $filePath . '\', \'_blank\')" 
...
document.getElementById("confirmDeleteBtn").addEventListener("click", function() {
    ...
    .catch(err => console.error('Error deleting remark:', err));
                                    ...
                                    <form method="POST"  style="display:inline;">
                                        <input type="hidden" name="ref_id" value="<?php echo htmlspecialchars($row['ref_id']); ?>">
                                        <button type="submit" class="btn btn-primary" style="position:absolute;right: 10px;top: 10px;display: flex;">
...
?>
    ...
    <div>
        ...
        <div style="padding-top: 1px;padding-left: 3px;">Back</div>
        </button>
    </div>
...
<script>
// Auto-scan all documents on page load
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(function() {
        ...
        documentsToScan.forEach(function(docElement) {
            ...
            scanQRCode(filePath, fileExtension, containerId);
        });
    }, 500);
});

// Main QR Code Scanner Function
function scanQRCode(filePath, fileExtension, containerId) {
    ...
    if (imageExtensions.includes(fileExtension.toLowerCase())) {
        ...
        showResult(containerId, 'info', 'ℹ File type not supported for QR scanning');
    }
}
...
function scanImageFile(filePath, containerId) {
    ...
    img.onload = function() {
        try {
            ...
            if (code) {
                checkQrCodeInDatabase(code.data, containerId);
            } else {
                showResult(containerId, 'warning', '⚠ No QR code detected. Document verification failed.');
            }
        } catch (error) {
            console.error('Image scanning error:', error);
            showResult(containerId, 'error', '❌ Error scanning');
        }
    };
...
// Scan PDF Files
function scanPdfFile(filePath, containerId) {
    fetch(filePath)
        .then(response => {
            if (!response.ok) {
        ...
        .then(page => {
            ...
            return page.render({ canvasContext: context, viewport }).promise.then(() => {
                ...
                if (code) {
                    ...
                    showResult(containerId, 'warning', '⚠ No QR code found');
                }
            });
        })
        .catch(error => {
            console.error('PDF scanning error:', error);
            showResult(containerId, 'error', '❌ Error scanning PDF');
        });
}

// Check QR Code Against Database
function checkQrCodeInDatabase(qrValue, containerId) {
    ...
    .then(data => {
        ...
        if (data.success && data.found) {
            showResult(containerId, 'success', '✅ Document exists in the database');
        } else if (data.success && !data.found) {
            showResult(containerId, 'error', '❌ Document does not exist in the database');
        } else {
            showResult(containerId, 'error', '❌ Document verification failed.');
        }
    })
    .catch(error => {
