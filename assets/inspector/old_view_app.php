    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                // STEP 2: If status is "Replied", update it
...
// Enhanced QR Code and Text Scanner with Full Field Extraction
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(function() {
...
function scanImageFile(filePath, containerId) {
    ...
    img.onload = function() {
        try {
            ...
            let qrCodeValue = code ? code.data : null;
            ...
            // Step 2: QR code found - Now extract text using Tesseract.js
            ...
            Tesseract.recognize(
                ...
                console.log('Extracted Fields from Image:', {
                    qr_code: qrCodeValue,
                    owner: extractedFields.owner,
                    license_number: extractedFields.license_number,
                    application_type: extractedFields.application_type,
                    validity_license: extractedFields.validity_license
                });
                
                // Verify in database
                checkDocumentInDatabase(
                    qrCodeValue,
                    extractedFields.owner,
                    extractedFields.license_number,
                    extractedFields.application_type,
                    extractedFields.validity_license,
                    containerId
                );
                
            }).catch(function(error) {
                console.error('OCR error:', error);
                // If OCR fails, still verify QR code only
                checkDocumentInDatabase(qrCodeValue, null, null, null, null, containerId);
            });
            
        } catch (error) {
            console.error('Image scanning error:', error);
            showResult(containerId, 'error', '❌ Scanning error');
        }
    };
...
// Scan PDF Files with Text Extraction
async function scanPdfFile(filePath, containerId) {
    try {
        ...
        // Scan all pages for QR and text
        for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
            ...
            // Method 1: Get text items with their positions (preserves layout)
            let pageTextStructured = '';
            let lastY = 0;
            ...
            textContent.items.forEach(item => {
                ...
                pageTextStructured += item.str + ' ';
                lastY = item.transform[5];
            });
            
            textByPage.push(pageTextStructured);
        ...
        // CHECK IF QR CODE EXISTS - If not, stop here
        if (!qrCodeValue) {
            showResult(containerId, 'warning', 'No QR code attached');
            return;
        }
        
        // QR CODE FOUND - Now extract fields!
        let extractedFields = extractFieldsFromText(textByPage.join('\n'));
        console.log('Extracted Fields from Document:', {
    qr_code: qrCodeValue,
    owner: extractedFields.owner,
    license_number: extractedFields.license_number,
    application_type: extractedFields.application_type,
    validity_license: extractedFields.validity_license
        ...
        // If any field is missing, try with simple concatenation
        if (!extractedFields.owner || !extractedFields.license_number || 
            !extractedFields.application_type || !extractedFields.validity_license) {
            const fallbackFields = extractFieldsFromText(fullText);
            
            // Fill in missing fields
            if (!extractedFields.owner && fallbackFields.owner) extractedFields.owner = fallbackFields.owner;
            if (!extractedFields.license_number && fallbackFields.license_number) extractedFields.license_number = fallbackFields.license_number;
            if (!extractedFields.application_type && fallbackFields.application_type) extractedFields.application_type = fallbackFields.application_type;
            if (!extractedFields.validity_license && fallbackFields.validity_license) extractedFields.validity_license = fallbackFields.validity_license;
        }
...
function extractFieldsFromText(text) {
    ...
    const fields = {
        ...
        validity_license: null
    };
    
    // Extract Owner
    const ownerPatterns = [
        /OWNER\s*:?\s*([A-Za-z\s\.\-']+?)(?=\s+LICENSE\s+NUMBER|LICENSE NUMBER|\s+APPLICATION\s+TYPE|APPLICATION TYPE|\s+VALIDITY|$)/i,
        /Owner\s*:?\s*([A-Za-z\s\.\-']+?)(?=\s+License\s+Number|License Number|\s+Application\s+Type|Application Type|\s+Validity|$)/i
    ];
    
    for (let pattern of ownerPatterns) {
        const match = text.match(pattern);
        if (match && match[1].trim().length >= 1) {
            fields.owner = match[1].trim().replace(/\s+/g, ' ');
            break;
        }
    }
    ...
    const licensePatterns = [
        /LICENSE\s+NUMBER\s*:?\s*([A-Z0-9\s\-]+?)(?=\s+APPLICATION\s+TYPE|APPLICATION TYPE|\s+VALIDITY|OWNER|$)/i,
        /License\s+Number\s*:?\s*([A-Z0-9\s\-]+?)(?=\s+Application\s+Type|Application Type|\s+Validity|Owner|$)/i,
        /LICENSE\s*NUMBER\s*:?\s*([0-9]+)/i,
        /License\s*Number\s*:?\s*([0-9]+)/i
    ];
    ...
    for (let pattern of licensePatterns) {
        const match = text.match(pattern);
        if (match && match[1].trim().length >= 1) {
            fields.license_number = match[1].trim().replace(/\s+/g, ' ');
            break;
        }
    }
    ...
    const appTypePatterns = [
        /APPLICATION\s+TYPE\s*:?\s*([A-Za-z\s]+(?:License)?[A-Za-z\s]*)(?=\s+VALIDITY\s+LICENSE|VALIDITY LICENSE|\s+OWNER|OWNER|LICENSE\s+NUMBER|$)/i,
        /Application\s+Type\s*:?\s*([A-Za-z\s]+(?:License)?[A-Za-z\s]*)(?=\s+Validity\s+License|Validity License|\s+Owner|Owner|License\s+Number|$)/i,
        /APPLICATION\s*TYPE\s*:?\s*((?:[A-Za-z]+\s+)*License)/i,
        /Application\s*Type\s*:?\s*((?:[A-Za-z]+\s+)*License)/i
    ];
    
    for (let pattern of appTypePatterns) {
        const match = text.match(pattern);
        if (match && match[1].trim().length > 2) {
            fields.application_type = match[1].trim().replace(/\s+/g, ' ');
            break;
        }
    }
    
   // Extract Validity License - Enhanced patterns for better accuracy
  // Extract Validity License - Enhanced patterns for better accuracy
    const validityPatterns = [
        // Pattern 1: Full date range with various dash types and spacing
        /VALIDITY\s+LICENSE\s*:?\s*((?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}\s*,?\s*\d{4}\s*[\s–\-—−]*\s*(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}\s*,?\s*\d{4})/i,
        /Validity\s+License\s*:?\s*((?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}\s*,?\s*\d{4}\s*[\s–\-—−]*\s*(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}\s*,?\s*\d{4})/i,
        
        // Pattern 2: More flexible - captures everything after "VALIDITY LICENSE:" until next field or end
        /VALIDITY\s+LICENSE\s*:?\s*([A-Za-z0-9\s,–\-—−.]+?(?:\d{4}))/i,
        /Validity\s+License\s*:?\s*([A-Za-z0-9\s,–\-—−.]+?(?:\d{4}))/i,
        
        // Pattern 3: Very permissive - captures any text with dates after VALIDITY LICENSE
        /VALIDITY\s*LICENSE\s*:?\s*([^\n]+\d{4}[^\n]*)/i,
        /Validity\s*License\s*:?\s*([^\n]+\d{4}[^\n]*)/i
    ];
    
    for (let pattern of validityPatterns) {
        const match = text.match(pattern);
        if (match && match[1].trim().length > 10) {
            fields.validity_license = match[1].trim().replace(/\s+/g, ' ');
            break;
        }
    }
...
function checkDocumentInDatabase(qrCode, owner, licenseNumber, applicationType, validityLicense, containerId) {
    const requestData = {};
    
     if (qrCode) requestData.qr_code = qrCode; // QR codes usually don't need normalization
    if (owner) requestData.owner = normalizeText(owner);
    if (licenseNumber) requestData.license_number = normalizeText(licenseNumber);
    if (applicationType) requestData.application_type = normalizeText(applicationType);
    if (validityLicense) requestData.validity_license = normalizeText(validityLicense);
    ...
    .then(data => {
        if (data.success && data.found) {
            ...
            if (data.match_details && data.match_details.field_matches) {
                ...
                resultHtml += '<div style="font-size: 11px; margin-top: 5px; text-align: left;">';
                
                // Always display all 5 fields
                ...
                showResultHtml(containerId, resultType, resultHtml);
            } else {
                showResult(containerId, 'success', '✅ Document found in database');
            }
        } else if (data.success && !data.found) {
            let resultHtml = '<strong style="font-size: 11px;">❌ QR Code: Not in database</strong><br>';
            resultHtml += '<div style="font-size: 11px; margin-top: 5px; text-align: left;">';
            ...
            resultHtml += licenseNumber 
                ? '' 
                : '';
            
            resultHtml += applicationType 
                ? '' 
                : '⚪ App Type: Not scanned<br>';
            
            resultHtml += validityLicense 
                ? '' 
                : '⚪ Validity: Not scanned<br>';
            
            resultHtml += '</div>';
            showResultHtml(containerId, 'error', resultHtml);
        } else {
            showResult(containerId, 'error', '❌ Verification failed');
        }
    })
    .catch(error => {
        console.error('Verification error:', error);
        showResult(containerId, 'error', '❌ Connection error');
    });
}

// Display Results (Simple Text)
