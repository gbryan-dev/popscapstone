...
fetch('api/get_monthly_stats.php')
    // remove ?year=2025 → we let PHP decide the year automatically
    .then(response => response.json())
    .then(data => {
        console.log(data); // You will now see real data

        var options3 = {
            chart: { height: 350, type: 'bar' },
            colors: ['#06BA54', '#D52941', '#0C0D50'],
            plotOptions: {
            ...
            series: [
                { name: 'Permit Issued',   data: data.issued },
                { name: 'Permit Rejected',  data: data.rejected },
                { name: 'All Clients',     data: data.clients }
            ],
            xaxis: {
            ...
            title: { text: `${data.year} Permit Processing Statistics`, align: 'center', style: {
                    fontSize: '16px',
                    fontWeight: 'bold',
                    color: '#0C0D50',        // your red color
                    fontFamily: 'Poppins, Arial, sans-serif'  // optional: use your theme font
                } },
            fill: { opacity: 1 },
    ...
    .catch(err => {
        ...
        document.querySelector("#apex3").innerHTML = "<p style='color:red>Failed to load data. Check console.</p>";
    });
