...
$sql = "
    SELECT p.ref_id, p.permit_for, p.application_id, p.client_id, p.status, p.apply_date,
...
if ($result && $result->num_rows > 0) {
    ...
    default:
        ...
        if ($status == 'Permit Issued') {
            echo '<form method="POST" action="spdf">
        ...
        echo '<button type="button" class="btn btn-danger btn-sm" 
            onclick="openDeleteApplicationModal(\'' . $refId . '\', \'' . addslashes($permitFor) . '\', \'' . addslashes($clientName) . '\')">
        Delete
      </button>
    </td>
</tr>';
