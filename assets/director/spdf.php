...
$application_id = (int)$_POST['application_id']; // It's an INT in both tables!
...
$stmt->bind_param("i", $application_id);  // "i" = integer → correct type!
$stmt->execute();
...
if ($result->num_rows === 0) {
    ...
    <body>
        <form id="redirectForm" method="POST" action="rejected_byinspector.php">
            <input type="hidden" name="application_id" value="<?php echo $application_id; ?>">
            <p>Redirecting to application status...</p>
...
.page {margin:5px 0; }
.page svg {background-color:#fff; margin-top: 40px;}
        ...
        <div class="modal2" onclick="event.stopPropagation()">
            <div class="boxcon">
                <div>
                    ...
                    <div class="countdown-container">
                        <div class="countdown-text">This page will auto-redirect after 10 secs</div>
                    </div>
                </div>
            </div>
        </div>
...
<div class="d-flex gap-2" style="justify-content: center;margin-top: 20px;margin-bottom:300px;font-family: 'Poppins';">
    <!-- Back Button -->
    <!-- Edit Button -->
    ...
    <?php if ($status != 'Rejected'): ?>
                                        ...
                                        <button type="button"  onclick="permitrejected('<?php echo $application_id; ?>')" class="btn text-white" style="display: flex;height:38px;background: #C5131E !important;">
      ...
      <form id="viewpermitbtn" action="view_application" method="POST" style="display:inline;">
          <input type="hidden" name="ref_id" value="<?php echo $ref_id; ?>">
        <button type="submit"  class="btn text-white btn-primary" style="height:38px;display: flex;">
            <div style=""><i class="material-icons" style="font-size: 17px;margin-top: 3px">edit</i> &nbsp;Edit</div>
        </button>
    </form>
   ...
   <button type="button" 
        id="approveBtn_<?php echo $application_id; ?>"
        onclick="endorsetodirector('<?php echo $application_id; ?>')" 
        class="btn" 
        style="height:38px;background: #19E915 !important;color: white !important;">
    <i style="font-size: 15px;" class="material-icons">check</i> Approve
</button>
...
function permitrejected(appID) {
    const bodyData = new FormData();
    bodyData.append('appID', appID);
    
    fetch('api/permitrejected.php', {
        method: 'POST',
        body: bodyData  // FormData doesn't need toString() or Content-Type header
    })
    .then(response => response.text())
    .then(data => {
      showModal('successrejected')
startCountdown()
...
function endorsetodirector(appID) {
    // Get the button element
    const btn = document.getElementById('approveBtn_' + appID);
    
    // Disable button and change text
    btn.disabled = true;
    btn.style.opacity = '0.6';
    btn.style.cursor = 'not-allowed';
    btn.innerHTML = '<i style="font-size: 15px;" class="material-icons">hourglass_empty</i> Approving...';
    ...
    const now = new Date();
    // Format current time
    const currentTime = new Intl.DateTimeFormat('en-PH', options).format(now);
    
   const validityDate = new Date();
validityDate.setDate(validityDate.getDate() + 3);    // ← this is the correct method

// Format for Philippines
const formattedValidity = new Intl.DateTimeFormat('en-PH', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
    timeZone: 'Asia/Manila'
    ...
    bodyData.append('appID', appID);
    bodyData.append('current_time', currentTime);
    bodyData.append('valid_until', formattedValidity);
    
    let client_id = '<?php echo $client_id; ?>';
    let ref_id = '<?php echo $ref_id; ?>';
    bodyData.append('client_id', client_id);
    bodyData.append('ref_id', ref_id);
    
    fetch('api/update_permit_issued.php', {
        method: 'POST',
    ...
    .then(response => response.text())
    .then(data => {
        showModal('successendorsed');
        celebrateConfetti('successendorsed_confetti');
        startCountdown();
        
        // Keep button disabled after success
        btn.innerHTML = '<i style="font-size: 15px;" class="material-icons">check_circle</i> Approved';
    ...
    .catch(err => {
        ...
        btn.innerHTML = '<i style="font-size: 15px;" class="material-icons">check</i> Approve';
