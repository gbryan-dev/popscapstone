...
function generateRefId() {
    ...
    return "MANTFPD{$year}{$monthDay}{$shuffledMilliseconds}";
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $client_id = $_SESSION['client_id'];
     ...
     $permit_for = 'Transport Firecrackers and Pyrotechnic Devices';
    $apply_date = mysqli_real_escape_string($conn, $_POST['current_time']);
    
    // Insert application data into the database
    $sql = "INSERT INTO applications (client_id, ref_id, permit_for, apply_date) 
            VALUES ('$client_id', '$ref_id', '$permit_for', '$apply_date')";
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        $files = [
            ...
            'dealers_license' => $_FILES['dealers_license'],
            'proof_of_purchased' => $_FILES['proof_of_purchased'],
