...
function generateRefId() {
    ...
    return "RET{$year}{$monthDay}{$shuffledMilliseconds}";
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $client_id = $_SESSION['client_id'];
     ...
     $permit_for = 'Permit To Sell Firecrackers And Pyrotechnics Devices';
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        $files = [
            'letter_request' => $_FILES['letter_request'],
            'distributors_certificate' => $_FILES['distributors_certificate'],
            'photocopy_license' => $_FILES['photocopy_license'],
            'barangay_police_court_clearance' => $_FILES['barangay_police_court_clearance'],
            'fireworks_training_certificate' => $_FILES['fireworks_training_certificate'],
        ...
        foreach ($files as $field_name => $file_array) {
            ...
            if (isset($file_array['name']) && is_array($file_array['name'])) {
                ...
                for ($i = 0; $i < count($file_array['name']); $i++) {
                    ...
                    $allowed_extensions = ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp', 'tiff', 'heic', 'pdf', 'docx', 'xlsx', 'pptx'];
